...
df_raw = pd.read_csv('resultados_exclusivos.csv')

# 1. Agrupamento incluindo o Kernel
agrupadores = ['Resolucao', 'Iteracoes', 'Modo', 'Kernel']
df = df_raw.groupby(agrupadores)['Tempo'].mean().reset_index()

# 2. Cálculo de Speedup e Eficiência
df_t1 = df[df['Modo'] == 'Serial'][['Resolucao', 'Iteracoes', 'Kernel', 'Tempo']].rename(columns={'Tempo': 'T1'})
df = df.merge(df_t1, on=['Resolucao', 'Iteracoes', 'Kernel'])
df['Speedup'] = df['T1'] / df['Tempo']
df['Eficiencia'] = df['Speedup'] / df['Modo'].map({'Serial': 1, 'Paralelo': 2})
