        ...
        for (int j = 0; j < image_size_w; j++){
            ...
            for (int k = 0; k < kernel_size; k++){
                for (int l = 0; l < kernel_size; l++){
                    // Calcula a posição real na imagem original
                    int img_i = i + k - pad_h;
                    int img_j = j + l - pad_w;

                    // Aplica o clamp para tratar as bordas
                    img_i = CLAMP(img_i, 0, image_size_h - 1);
                    img_j = CLAMP(img_j, 0, image_size_w - 1);
                    
                    sum += image[img_i][img_j] * kernel[k][l];
                }                                    
            }
            output[i][j] = sum;
        }
